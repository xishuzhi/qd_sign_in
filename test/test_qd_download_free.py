...
class UtilsTools:
    ...
    # 替换标题不用做路径和文件名
    def replace_title(self, text):
        t = self.make_dict('ａｂｃｄｅｆｇｈｉｊｋｌｍｎｏｐｑｒｓｔｕｖｗｘｙｚＡＢＣＤＥＦＧＨＩＪＫＬＭＮＯＰＱＲＳＴＵＶＷＸＹＺ１２３４５６７８９０，．！?!\n',
    ...
    # 替换字符串
    def replace_file_path(self, path):
        path = path.replace('/', '-')
        path = path.replace('\\', '-')
        path = path.replace('*', '-')
        path = path.replace('?', '？')
    ...
    def path_format(self, path):
        if os.name == 'nt':
            path = self.path_win(path)
        elif os.name == 'Android' or os.name == 'posix':
            path = self.path_linux(path)
        return path

    def save_gzip(self, path, data):
        try:
            path = self.path_format(path)
